...
function preprocessFrame(payload: RunPoseFramePayload) {
  ...
  if (rgba.length < expectedLength) {
    throw new Error(
      `Invalid RGBA buffer length: expected ${expectedLength}, got ${rgba.length}`,
    );
  }
...
function parseBestPose(
  ...
  if (itemSize < 5 + KEYPOINT_VALUE_COUNT) {
    throw new Error(
      `Expected YOLO pose item size >= ${5 + KEYPOINT_VALUE_COUNT}, got ${itemSize}`,
    );
  }
...
export function registerPoseIpc() {
  ...
  ipcMain.handle(
    "pose:run-frame",
    async (_event, payload: RunPoseFramePayload) => {
