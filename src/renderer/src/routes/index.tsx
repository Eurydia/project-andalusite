...
export const Route = createFileRoute("/")({
  ...
  loader: async ({ deps }) => {
    ...
    const onboardingHasRun = await window.persist.get(
      "onboard-home-has-run",
      false,
    );
    const onboardingCardHasRun = await window.persist.get(
      "onboard-card-has-run",
      false,
    );
...
function RouteComponent() {
  ...
  const [onboardHomeHasRun, setOnboardHomeHasRun] = useState(
    data.onboardingHasRun ?? false,
  );
  const [onboardCardHasRun, setOnboardCardHasRun] = useState(
    data.onboardingCardHasRun ?? false,
  );
