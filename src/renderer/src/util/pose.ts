...
function getAverageScore(keypoints: Keypoint[]) {
  ...
  return (
    keypoints.reduce((total, point) => total + point.score, 0) /
    keypoints.length
  );
}

export function getDownwardDogMetrics(
  keypoints: Keypoint[],
): DownwardDogMetrics | null {
  ...
  let downwardDogStatus: DownwardDogMetrics["downwardDogStatus"] =
    "Good Downward Dog";
...
export function getDownwardDogFeedback(
  ...
  if (!metrics) {
    return {
      ...
      message:
        "Keep your shoulders, elbows, wrists, hips, knees, and ankles visible.",
    };
  }
